...
- `1x1`: 16 个 `64x64` 裁切，作为稳定底纹。
- `2x2`: 16 个 `128x128` 裁切，低频变化比 1x1 略多，但不能明显盖成方块。
- `4x4`: 16 个 `256x256` 裁切，用来承载大区块差异。

关键处理：

- 裁切位置要分散，不要从左到右顺序扫描，避免在 atlas 行里形成条纹。
- 小贴图做较强色调归一，保持循环稳定。
- `4x4` 的中心可以保留更强的大区块变化，边缘要压回共同平均色和细节频率。
- 预览必须做混铺压力测试：先铺满 `1x1`，再盖多个 `2x2` 和 `4x4`，看是否露出矩形边。
